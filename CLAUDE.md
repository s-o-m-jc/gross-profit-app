...
**定義(はまさんの確認済み)**: 交通費(税抜)は、派遣先企業に交通費を請求する際、税抜きで計算
してから請求小計に合算し消費税を乗せて請求する、という運用上の金額。派遣スタッフの交通費は
「相手企業負担(派遣先へ請求)」か「自社負担」のいずれかに分かれ、交通費(税抜)は必ずどちらかの
スタッフ別実額と一致する(相手企業負担額そのもの、自社負担額そのもの、またはそのいずれかを
1.1で割った額。四捨五入により1円の誤差が生じることがある)。このアプリが持つどのデータからも
算出できない外部由来の値のため、対象月ごとの手入力上書き値(`TransportExTaxOverrideRow`、
「紹介手数料・その他調整」タブの「交通費(税抜)」サブタブ、大阪選択時のみ表示)として保持する。

**★データソースの注意(重要・必ず個別ファイルを参照すること)**: 「月別総合計」ファイルの
「集計」タブ自体にも同名の列があるが、2024-07/08/09・2024-12・2025-04・2025-06の6ヶ月は
誤入力(自社負担額を代わりに入力)されており信頼できないことが判明している。**正しい値は
各月の個別ファイル(契約別売上実績表)内の対象月シートで、スタッフごとの「交通費（税抜）」列を
合計した値**。今後この値を追加・修正する場合は、必ず個別ファイル側を参照すること
(「集計」タブの値をそのまま転記しないこと)。

**対象範囲**: 大阪のみ(2026-09-25時点)。四国は勤怠明細の「交通費チェック」シート、松山は
非課税扱いとのことで、管理方法が異なるため対象外(別途検討が必要になった場合に対応)。
月次粗利明細一覧(個別契約行)の同名列には反映せず、決算期(年間)集計・グラフの月次サマリ・
年間合計にのみ反映する(スコープを広げすぎないための意図的な制限)。

**2023-07〜2023-09分**: 元データに列自体が無いため対象外。今後も「不明」表示のまま。
