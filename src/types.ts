...
export interface InvoicePrintRow {
  ...
  unitPrice: number;
  // ★2026-09-29追加(はまさんの指摘・大阪2025-04の実データで確定): 受注番号(「受注番号」列)。
  // 請求No(billingNo)は、大阪の実データでは「1つの請求Noに複数スタッフ分の受注がまとめて
  // 紐づく」(1つのクライアントへの合算請求)ケースがあり、契約(スタッフ)単位の一意な識別子には
  // ならない場面がある。さらに、請求No自体が(管理用の参照番号のため)空欄のまま計上される行も
  // 実在する(田中千恵さん他、株式会社ブンカの契約8件で確認)。受注番号は契約(スタッフ)単位で
  // 一意なため、請求No空欄時のフォールバック結合キーとしてcalculator.tsで使用する
  // (billingNoが取得できる場合は従来通りbillingNoでの結合を優先する)。
  orderNo?: string;
}
